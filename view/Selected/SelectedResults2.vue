          ...
          <div class="col-7 ">
            <div class="row flex-column justify-content-between">
              <div class="card-body pe-4">
                <!-- 4.4 標題 -->
                <div class="col mt-3">
                  <h6 class="fw-bold text-truncate">{{item.RestaurantName || item.HotelName}}</h6>
                </div>
                <!-- 4.5 內文 -->
                ...
                <div class="col mb-2">
                  ...
                  flex-column">
                    <i class="bi bi-geo-alt-fill location text-truncate">
                      <span class="text-dark fw-bold px-2">{{ item.Location || item.City || item.Address }}</span>
                    </i>
...
// 1. Pinia <SELECT> 指定篩選
  ...
  const {select, getAssetUrl} = selectStore;
  const {selectedOption1,
  selectedOption2,
  options1,
  allCityArr,
  dataArr,
  isLoading} = storeToRefs(selectStore);
...
const paginatedArr = computed(() => {
  ...
  return dataArr.value.slice(startIndex,endIndex);
});
// 4. 上一頁
...
const totalPages = computed(() => {
  return Math.ceil(dataArr.value.length / itemsPerPage.value);
});
